...
[cropped_TV, w1, w2, Energy, Dgap, TimeCost, itr] = ...
  ...
  filtered_cropped(:,:,Ind+2) = cropped_TV;
...
[~, cropped_BM3D] = BM3D(cropped_I_norm_unlog, cropped_I_norm_unlog,sigma); %% %% F'_,7,8,9
 filtered_cropped(:,:,Ind+6) = cropped_BM3D;
end
...
%        STTF(:,:,i)=(STT1(:,:,i,j)+STT2(:,:,i,j)+STT3(:,:,i,j)+STT4(:,:,i,j)+STT5(:,:,i,j))/5;
   ...
   ROI_B=ROI_B(:,:,i);
EPI_1 = epi(ROI_B,ROI_1);
EPI_2 = epi(ROI_B,ROI_2);
EPI_3 = epi(ROI_B,ROI_3);
EPI_4 = epi(ROI_B,ROI_4);
EPI_5 = epi(ROI_B,ROI_5);
EPI(i)=EPI_1+EPI_2+EPI_3+EPI_4+EPI_5/5;
